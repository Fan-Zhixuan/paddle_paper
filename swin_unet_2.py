...
class SwinUnet(nn.Layer):
    ...
    Args:
        ...
        num_classes (int): Number of classes for classification head. Default: 19
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
                 ...
                 num_classes=19, with_pool=True, **kwargs):
        super().__init__()
        self.class_dim = num_classes
        self.with_pool = with_pool

        self.num_downlayers = len(depths)
        self.num_uplayers = len(depths)-1
        ...
        # build layers
        self.downlayers = nn.LayerList()
        for i_layer in range(self.num_downlayers):
            layer = BasicDownsampleLayer(dim=int(embed_dim * 2 ** i_layer),
            ...
            self.downlayers.append(layer)
        self.uplayers = nn.LayerList()
        for i_layer in range(self.num_uplayers):
            layer = BasicUpsampleLayer(dim=int(embed_dim * 2 ** (2-i_layer)),
        ...
        self.patch_expanding = PatchExpanding(input_resolution=(patches_resolution[0],
    ...
    def forward(self, x):
        ...
        x = self.pos_drop(x)
        # print('x',x.shape)
        x01 = self.downlayers[0](x)
        # print('x01',x01.shape)
        x02 = self.downlayers[1](x01)
        # print('x02',x02.shape)
        x04 = self.downlayers[2](x02)
        # print('x04',x04.shape)
        x08 = self.downlayers[3](x04)
        # print('x08',x08.shape)
        x14 = self.uplayers[0](x08)
        x14 = x14+x04
        # print('x14',x14.shape)
        x12 = self.uplayers[1](x14)
        x12 = x12+x02
        # print('x12',x12.shape)
        x11 = self.uplayers[2](x12)
        x11 = x11+x01
        # print('x11', x11.shape)
        
        x_out = self.patch_expanding(x11)
        # print('x',x.shape)
        if self.class_dim > 0:
...
class PatchExpanding(nn.Layer):
    ...
    def __init__(self, input_resolution, dim, Expanding_scale=2):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.scale = Expanding_scale
        assert Expanding_scale == 2 or Expanding_scale == 4, "get wrong upsample scale."
  
        if self.scale == 2:
            self.linear = nn.Linear(dim, 2 * dim, bias_attr=False)
            self.norm = nn.LayerNorm(dim//2)
        else:
            self.linear = nn.Linear(dim, 16 * dim, bias_attr=False)
    ...
    def forward(self, x):
        ...
        if self.scale == 2:
            ...
            x = x.reshape([B, int(4*H*W), int(C/2)])
            x = self.norm(x)
        else:
            x = x.reshape([B, 4*H, 4*W, C])
            x = self.norm(x)
       
        return x


class BasicDownsampleLayer(nn.Layer):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.swin_dim = dim
        self.merg_dim = dim//2
        self.swin_input_resolution = input_resolution
        self.merg_input_resolution = (input_resolution[0]*2, input_resolution[1]*2)
        self.depth = depth
        # build blocks
        self.blocks = nn.LayerList([
            SwinTransformerBlock(dim=self.swin_dim, 
                                 input_resolution=self.swin_input_resolution,
        ...
        if downsample is not None:
            self.downsample = downsample(
                self.merg_input_resolution, dim=self.merg_dim, norm_layer=norm_layer)
        else:
...
class BasicUpsampleLayer(nn.Layer):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of input channels.
        input_resolution (tuple[int]): Input resolution.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        super().__init__()
        self.expand_dim = dim*2
        self.swin_dim = dim
        self.expand_input_resolution = (input_resolution[0]//2, input_resolution[1]//2)
        self.swin_input_resolution = input_resolution
        self.depth = depth
        # build blocks
        self.blocks = nn.LayerList([
        ...
        if upsample is not None:
            self.upsample = upsample(self.expand_input_resolution,
                                     dim=self.expand_dim)
        else:
            self.upsample = None
...
model = swin_Unet_256(class_dim = 10)
model = paddle.Model(model)
model.summary((8, 3, 256, 256))