from swin import PatchExpanding
import numpy as np
...
class PatchExpanding(nn.Layer):
    ...
    def __init__(self, input_resolution, dim, Expanding_scale=2):
        ...
        assert Expanding_scale == 2 or Expanding_scale == 4,f"get wrong upsample scale"
...
class SwinUnet(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
        ...
        for i_layer in range(self.num_layers-1):
            layer = BasicUpsampleLayer(dim=int(embed_dim * 2 ** i_layer),
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer)),
                               depth=depths[i_layer],
                               num_heads=num_heads[i_layer],
            ...
            self.downlayers.append(layer)
        self.patch_expanding = PatchExpanding(input_resolution = (patches_resolution[0] // 4,
    ...
    def forward(self, x):
        ...
        x = self.pos_drop(x)
        x1 = self.downlayers[0](x)
        x2 = self.downlayers[1](x1)
        x3 = self.downlayers[2](x2)
        x4 = self.downlayers[3](x3)
        x5 = self.uplayers[0](x4)
        x5 = x5+x1
        x6 = self.uplayers[1](x5)
        x6 = x6+x2
        x7 = self.uplayers[2](x6)
        x7 = x7+x3
        x7 = self.patch_expanding(x7)
