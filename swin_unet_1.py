...
class BasicUpsampleLayer(nn.Layer):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if upsample is not None:
            self.upsample = upsample(self.input_resolution, dim=dim)
        else:
            self.upsample = None
...
class SwinUnet(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
        ...
        self.num_downlayers = len(depths)-1
        self.num_uplayers = len(depths)
        self.embed_dim = embed_dim
        self.ape = ape
        ...
        for i_layer in range(self.num_uplayers):
            layer = BasicUpsampleLayer(dim=int(embed_dim * 2 ** (3-i_layer)),
                                       input_resolution=(patches_resolution[0] // (2 ** (3-i_layer)),
                                                         patches_resolution[1] // (2 ** (3-i_layer))),
                                       depth=depths[i_layer],
                                       num_heads=num_heads[i_layer],
                                       window_size=window_size,
                                       mlp_ratio=self.mlp_ratio,
                                       ...
                                       upsample=PatchExpanding if (
                                                i_layer < self.num_uplayers - 1) else None
                                       )
